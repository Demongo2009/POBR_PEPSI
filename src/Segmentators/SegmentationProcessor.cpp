...
bool SegmentationProcessor::checkRed(cv::Vec3b &vec) {
	...
	if((hsl.h < 30 || hsl.h > 330) && hsl.s > 0.5 && hsl.l < 0.80){
		return true;
	}
	return false;
}
...
bool SegmentationProcessor::checkBlue(cv::Vec3b &vec) {
	...
	if(hsl.h < 280 && hsl.h > 180 && hsl.s > 0.4 && hsl.l < 0.7){
		return true;
	}
	return false;
}
...
void SegmentationProcessor::showSegments(SegmentVector vector1, cv::Mat& mat, ColorMat & colorMat) {
	...
	std::for_each(vector1.begin(),vector1.end(),[&](std::vector<std::pair<int, int>> segment){
		...
		std::for_each(segment.begin(), segment.end(), [&](std::pair<int, int> point){
			...
			switch (colorMat[x][y]) {
				case RED:
					filtered_[y][x][0] = (uchar)(std::max(std::min((int)0,255),0));
					filtered_[y][x][1] = (uchar)(std::max(std::min((int)0,255),0));
					filtered_[y][x][2] = (uchar)(std::max(std::min((int)255,255),0));
					break;
				case BLUE:
					filtered_[y][x][0] = (uchar)(std::max(std::min((int)255,255),0));
					filtered_[y][x][1] = (uchar)(std::max(std::min((int)0,255),0));
					filtered_[y][x][2] = (uchar)(std::max(std::min((int)0,255),0));
					break;
				case WHITE:
					filtered_[y][x][0] = (uchar)(std::max(std::min((int)255,255),0));
					filtered_[y][x][1] = (uchar)(std::max(std::min((int)255,255),0));
					filtered_[y][x][2] = (uchar)(std::max(std::min((int)255,255),0));
					break;
				default:
					filtered_[y][x][0] = (uchar)(std::max(std::min((int)255,255),0));
	...
	cv::imshow("Segmentation", filtered);
}
